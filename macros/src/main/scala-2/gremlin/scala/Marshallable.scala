...
object Marshallable {
  ...
  def materializeMappableImpl[CC <: Product: c.WeakTypeTag](
    ...
    import c.universe._
    val tpe = weakTypeOf[CC] //Universe.Type
    val companion = tpe.typeSymbol.companion

    //Fold over fields in the case class
    
    val (idParam, fromCCParams, toCCParams) = tpe.decls //MemberScope <: Traversable
      .foldLeft[(Tree, Seq[Tree], Seq[Tree])]((q"_root_.scala.None", Seq.empty, Seq.empty)) {
        //filter for just the case class ctor params
        case ((_idParam, _fromCCParams, _toCCParams), field: MethodSymbol) if field.isCaseAccessor =>

          val name = field.name

          //Same as name, but with symbols substituted
          //Example: `foo_+=` becomes `foo_\$plus\$eq`
          //We use this for the marshalled property name
          ...
          /**
            * Return symbol for the first non-constructor method on @tpe
            */
          ...
          /**
            * Return symbol for the `apply` method on the type's companion, if defined
            */
          ...
          /**
            * Return Type of the first param to the `apply` method on the type's companion, if defined
            */
          ...
          /**
            * Process field with an `@id` annotation
            **/
          def handleId = {
            assert(
          ...
          /**
            * Process field with an `@underlying` annotation
            * */
