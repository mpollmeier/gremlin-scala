...
class BackStepTest extends com.tinkerpop.gremlin.test.filter.BackStepTest {
  ...
  def test_g_v4_out_filterXlang_eq_javaX_backX1X() {
    super.test_g_v4_out_filterXlang_eq_javaX_backX1X(g.v(4).out.filter { v: Vertex ⇒ v.get[String]("lang").exists(_ == "java") }.back(1))
  }

  def test_g_v4_out_asXhereX_filterXlang_eq_javaX_backXhereX() {
    super.test_g_v4_out_asXhereX_filterXlang_eq_javaX_backXhereX(g.v(4).out.as("here").filter { v: Vertex ⇒ v.get[String]("lang").exists(_.equals("java")) }.back("here").asInstanceOf[Pipe[Vertex, Vertex]]);
  }

  def test_g_v4_out_asXhereX_filterXlang_eq_javaX_backXhereX_propertyXnameX() {
    super.test_g_v4_out_asXhereX_filterXlang_eq_javaX_backXhereX_propertyXnameX(g.v(4).out.as("here").filter { v: Vertex ⇒ v.get[String]("lang").exists(_.equals("java")) }.back("here").property("name").asInstanceOf[Pipe[Vertex, String]]);
  }
}
