...
class FilterStepTest extends com.tinkerpop.gremlin.test.filter.FilterStepTest {
  ...
  def test_g_V_filterXlang_eq_javaX() {
    super.test_g_V_filterXlang_eq_javaX(g.V.filter { v: Vertex ⇒ v.get[String]("lang").exists(_.equals("java")) });
  }

  def test_g_v1_out_filterXage_gt_30X() {
    super.test_g_v1_out_filterXage_gt_30X(g.v(1).out.filter { v: Vertex ⇒ v.get[JInteger]("age").exists(_ > 30) });
  }

  def test_g_V_filterXname_startsWith_m_OR_name_startsWith_pX() {
    super.test_g_V_filterXname_startsWith_m_OR_name_startsWith_pX(g.V.filter { v: Vertex ⇒ v.get[String]("name").exists(n ⇒ n.startsWith("m") || n.startsWith("p")) });
  }
}