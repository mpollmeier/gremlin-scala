...
class ScalaGraph(val graph: Graph) {
  ...
  //def V: GremlinScala[Vertex, Vertex] = new GremlinScalaPipeline[ScalaGraph, Vertex].V(graph)
  def V = GremlinScala[Vertex, Vertex :: HNil](new GremlinPipeline(graph).V())
  //def V = GremlinScala[Vertex, ScalaGraph :: Vertex :: HNil].V(graph)

  /**iterate all edges */
  //def E = GremlinScalaPipeline[Edge, ScalaGraph :: Edge :: HNil].E(graph)
