...
class Steps[EndDomain, EndGraph, LabelsDomain <: HList, LabelsGraph <: HList](val raw: GremlinScala[EndGraph, LabelsGraph])(
  ...
  // def as(stepLabel: StepLabel[EndDomain])(implicit p: Prepend[Labels, EndDomain :: HNil]) =

  def dedup[NewSteps]()(implicit constr: Constructor.Aux[EndDomain, LabelsGraph, EndGraph, NewSteps]): NewSteps =
    constr(raw.dedup())
  ...
  def onRaw[NewSteps](fun: GremlinScala[EndGraph, LabelsGraph] => GremlinScala[EndGraph, LabelsGraph])(
    implicit constr: Constructor.Aux[EndDomain, LabelsGraph, EndGraph, NewSteps]): NewSteps =
    constr(fun(raw))
  ...
  def map[NewEndDomain, NewEndGraph, NewSteps <: StepsRoot](fun: EndDomain ⇒ NewEndDomain)(
    ...
    constr: Constructor.Aux[NewEndDomain, LabelsGraph, NewEndGraph, NewSteps]): NewSteps =
      constr {
  ...
  def flatMap[NewSteps <: StepsRoot](fun: EndDomain ⇒ NewSteps)(
    implicit
    constr: Constructor.Aux[NewSteps#EndDomain0, LabelsGraph, NewSteps#EndGraph0, NewSteps],
...
class NodeSteps[EndDomain <: DomainRoot, LabelsDomain <: HList, LabelsGraph <: HList](override val raw: GremlinScala[Vertex, LabelsGraph])(
  ...
  def aggregate[NewSteps](into: mutable.Buffer[EndDomain])(
    implicit constr: Constructor.Aux[EndDomain, LabelsGraph, Vertex, NewSteps]): NewSteps =
    constr(
