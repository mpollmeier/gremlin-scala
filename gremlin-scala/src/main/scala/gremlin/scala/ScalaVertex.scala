...
case class ScalaVertex(vertex: Vertex) extends ScalaElement[Vertex] {
  ...
  def ---(label: Label) = SemiEdge(vertex, label)

  def ---(label: Label, properties: (String, Any)*) = SemiEdge(vertex, label, properties.toMap)
