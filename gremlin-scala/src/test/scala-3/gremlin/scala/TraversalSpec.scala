...
class TraversalSpec extends AnyWordSpec with Matchers {
  ...
  "vertex steps".can {
    ...
    "follow the out vertices" in new Fixture {
      graph.V(1).out().values[String]("name").toSet() should equal(Set("vadas", "josh", "lop"))
      graph.V(1).out("knows").values[String]("name").toSet() should equal(Set("vadas", "josh"))

      graph.V(1).out().out().values[String]("name").toSet() should equal(Set("ripple", "lop"))
      graph.V(1).out().out("created").values[String]("name").toSet() should equal(Set("ripple", "lop"))
    }

    "follow the in vertices" in new Fixture {
      graph.V(3).in().values[String]("name").toSet() should equal(Set("marko", "josh", "peter"))
      graph.V(3).in("created").values[String]("name").toSet() should equal(
        Set("marko", "josh", "peter"))

      graph.V(3).in().in().values[String]("name").toSet() should equal(Set("marko"))
      graph.V(3).in().in("knows").values[String]("name").toSet() should equal(Set("marko"))
    }

    "follow both in and out vertices" in new Fixture {
      graph.V(4).both().values[String]("name").toSet() should equal(Set("marko", "ripple", "lop"))
      graph.V(4).both("knows").values[String]("name").toSet() should equal(Set("marko"))

      graph.V(4).both().both().values[String]("name").toSet() should equal(
        Set("marko", "lop", "peter", "josh", "vadas"))
      graph.V(4).both().both("knows").values[String]("name").toSet() should equal(Set("josh", "vadas"))
    }

    "follow out edges" in new Fixture {
      graph.V(1).outE().toSet().map(_.label) should equal(Set("knows", "created"))
      graph.V(1).outE("knows").toSet().map(_.label) should equal(Set("knows"))

      graph.V(1).out().outE().toSet().map(_.label) should equal(Set("created"))
      graph.V(1).out().outE("created").toSet().map(_.label) should equal(Set("created"))
    }

    "follow in edges" in new Fixture {
      graph.V(3).inE().toSet().map(_.label) should equal(Set("created"))
      graph.V(3).inE("created").toSet().map(_.label) should equal(Set("created"))

      graph.V(3).in().inE().toSet().map(_.label) should equal(Set("knows"))
      graph.V(3).in().inE("knows").toSet().map(_.label) should equal(Set("knows"))
    }

    "follow both edges" in new Fixture {
      graph.V(4).bothE().toSet().map(_.label) should equal(Set("created", "knows"))
      graph.V(4).bothE("knows").toSet().map(_.label) should equal(Set("knows"))

      graph.V(4).in().bothE().toSet().map(_.label) should equal(Set("knows", "created"))
      graph.V(4).in().bothE("knows").toSet().map(_.label) should equal(Set("knows"))
    }
    ...
    "valueMap specific key" in new Fixture {
      ...
      map.get("name") shouldEqual List("marko").asJava
    }

    "valueMap all keys" in new Fixture {
      val map = graph.V(1).valueMap.head()
      map.get("name") shouldEqual List("marko").asJava
      map.get("age") shouldEqual List(29).asJava
    }

    "elementMap specific key" in new Fixture {
      val res = graph.V(1).elementMap("name").head()
      res.get(T.id) shouldEqual 1
      res.get(T.label) shouldEqual "person"
      res.get("name") shouldEqual "marko"
    }
  ...
  "edge steps".can {
    ...
    //     //e(7).inV//.out.values[String]("name").toSet should equal(Set("vadas", "josh", "lop"))
    //     //graph.V(1).out("knows").values[String]("name").toSet should equal(Set("vadas", "josh"))
    //     //graph.V(1).out(1, "knows").values[String]("name").toSet should equal(Set("vadas"))

    //     //graph.V(1).out.out.values[String]("name").toSet should equal(Set("ripple", "lop"))
    //     //graph.V(1).out.out("created").values[String]("name").toSet should equal(Set("ripple", "lop"))
    //     //graph.V(1).out.out(1, "created").values[String]("name").toSet should equal(Set("lop"))
    //   }
  ...
  "head".can {
    "get the first element" in new Fixture {
      graph.V().values[String]("name").head() shouldEqual "marko"
    }
  ...
  "headOption".can {
    "get the first element" in new Fixture {
      graph.V().values[String]("name").headOption() shouldEqual Some("marko")
    }
    ...
    "return None if there is no result" in new Fixture {
      graph
        ...
        .filter(_.has(Key("nonExistingProperty")))
        .values[String]("name")
        .headOption() shouldEqual None
    }
  }

  "value gets values" in new Fixture {
    graph.V().value(Age).toSet() shouldEqual Set(27, 29, 32, 35)
  }
    ...
    "order by multiple modulators" in new Fixture {
      val results: List[String] =
        graph
          ...
          .value(Name)
          .toList()
      results shouldEqual Seq("vadas", "peter", "marko", "josh")
    }
  }

  "map".can {
    "transform the latest step" in new Fixture {
      graph.V().map(_.label).toList() shouldEqual graph.V().label().toList()
    }
    ...
    "support for comprehension" in new Fixture {
      ...
      labels.toSet() shouldEqual graph.V().label().toSet()
    }
  }
  ...
  "flatMap".can {
    "transform the latest step" in new Fixture {
      ...
      graph.V(1).flatMap(_.outE()).toList() shouldEqual v1outEdges
    }
    ...
    "support for comprehension" when {
      "using simple case" in new Fixture {
        ...
        edgeLabels.toSet() shouldEqual graph.E().label().toSet()
      }
      ...
      "using slightly more complex case" in new Fixture {
        ...
        softwareAndDevAges.toSet() shouldEqual Set(
          "lop" -> 32d,
          "ripple" -> 32d
        )
      }
  ...
  "fold".can {
    "gather elements in a list" in new Fixture {
      ...
      result shouldEqual List("vadas", "josh").asJava
    }
  ...
  "limit in nested traversals" in {
    ...
    traversal.toMap shouldEqual Map(
      "michael" -> "scala",
      "marko" -> "groovy"
    )
  }
  ...
  "collect" in new Fixture {
    ...
    ages shouldEqual Set(27, 29, 32, 35)
  }
  ...
  "find all edges between two vertices" in {
    ...
    graph.E().count().head() shouldEqual 4
    val v0v1Edges = v0.bothE().filter(_.bothV().is(v1)).label().toSet()
    v0v1Edges shouldEqual Set("e0", "e1")
  }
  ...
  "`drop` removes elements from the graph" in new Fixture {
    ...
    graph.V().hasLabel("person").count().head() shouldEqual 2
    withClue("should remove corresponding edges as well") {
      graph.E().count().head() shouldEqual 2
    }
  }
  ...
  "groupCount" should {
    "work on values" in new Fixture {
      ...
      results.asScala shouldEqual Map(32 -> 1, 35 -> 1, 27 -> 1, 29 -> 1)
    }
  ...
  "dedup".can {
    "simply deduplicate" in new Fixture {
      graph.V().value(Lang).dedup().count().head() shouldEqual 1
    }

    "use by modulator" in new Fixture {
      graph.V().dedup(By.label).value(Name).toSet() shouldEqual Set("marko", "lop")
    }
  ...
  "subgraph" should {
    "work in simple scenario" in new Fixture {
      ...
      subGraph.V().count().head() shouldEqual 3
      subGraph.E().count().head() shouldEqual 2
    }
    ...
    "get all of the graph structure surrounding a single vertex" in new Fixture {
      ...
      subGraph.V().count().head() shouldEqual 4
      subGraph.E().count().head() shouldEqual 4
    }
  }
  ...
  "union supports heterogeneous queries" in new Fixture {
    ...
    outEdges.size shouldEqual 3
    outEdges.asScala.map(_.id).toSet shouldEqual Set(7, 8, 9)

    outVertices.size shouldEqual 3
    outVertices.asScala.map(_.id).toSet shouldEqual Set(2, 3, 4)
  }
  ...
  "unionFlat (only homogeneous queries)" should {
    "work for traversals with the same end type" in new Fixture {
      ...
      traversal.toSet() shouldEqual Set(27, 29, 32)
    }
    ...
    "falls back to `Any` for heterogeneous queries" in new Fixture {
      ...
      traversal.toSet() shouldEqual Set(29, "marko")
    }
  }
  ...
  "path" should {
    "include every step in the traversal" in new Fixture {
      ...
      path2String(results(0)) shouldEqual List("v[1]", "v[4]", "v[5]", "ripple")
      path2String(results(1)) shouldEqual List("v[1]", "v[4]", "v[3]", "lop")
    }
    ...
    "allow to be modulated" in new Fixture {
      ...
      results.size shouldEqual 2
      path2String(results(0)) shouldEqual List("marko", "32", "ripple")
      path2String(results(1)) shouldEqual List("marko", "32", "lop")
    }
    ...
    "only include steps between from and to" in new Fixture {
      ...
      val results: List[Path] =
        graph
          ...
          .as(knownPersonStep)
          .out(Created)
          .as(createdSoftwareStep)
          .path(By(Name))
          .from(knownPersonStep)
          .to(createdSoftwareStep)
          .toList()

      path2String(results(0)) shouldEqual List("josh", "ripple")
      path2String(results(1)) shouldEqual List("josh", "lop")
    }
  }
  ...
  "coalesce" should {
    "evaluate traversals and return first with value" when {
      ...
      "relationship is first" in new Fixture {
        ...
        traversal1.toList().map(path2String) shouldEqual
      ...
      "created is first" in new Fixture {
        ...
        traversal2.toList().map(path2String) shouldEqual Seq(Seq("marko", "created", "lop"))
      }
    }
    ...
    "handle vertex-unique properties" in new Fixture {
      ...
      traversal3.toSet() shouldEqual Set("okram", "vadas", "josh", "peter")
    }
    ...
    "allow constant as default" in new Fixture {
      ...
      traversal3.toSet() shouldEqual Set("35", "32", "27", "29", "ageless")
    }
  }
  ...
  "repeat/until step" should {
    ...
    "emit only results from final iteration by default" in new Fixture {
      ...
      traversal.toSet() shouldEqual Set("ripple", "lop")
    }
    ...
    "emit all intermediate results when using `emit` modulator" in new Fixture {
      ...
      traversal.toSet() shouldEqual Set("lop", "marko", "josh", "vadas", "ripple")
    }
    ...
    "use predicate in `until` modulator: iterate until no moure outgoing edges" in new Fixture {
      ...
      traversal.toSet().size shouldEqual 4
  ...
  "match step".can {
    "filter using pattern matching" in new Fixture {
      val traversal =
        graph
          ...
          .select("a", "c")
          .by("name")
      traversal.toSet() shouldEqual Set(
    ...
    "filter using recursive pattern matching" in new Fixture {
      val traversal = graph
        ...
        .select("a", "c")
        .by("name")
      traversal.toSet() shouldEqual Set(Map("a" -> "marko", "c" -> "ripple").asJava)
    }
  }
  ...
  "where step".can {
    "filter based on label" in new Fixture {
      ...
      traversal.toSet() shouldEqual Set("josh", "peter")
    }
    ...
    "filter with traversal" in new Fixture {
      ...
      traversal.toSet() shouldEqual Set("josh")
    }
    ...
    "be modulated with `by`" when {
      ...
      "comparing two labels" in new Fixture {
        ...
        traversal.toSet() shouldEqual Set("vadas")
      }
    }
  }
  ...
  "tree step".can {
    "generate a tree structure for a vertex" in {
      ...
      tree.keySet().asScala shouldEqual Set(vertices(1))

      val v1Edges = tree.values.asScala.head
      v1Edges.keySet.asScala shouldEqual Set(edges(0), edges(1))

      val verticesUnderV1 = v1Edges.values.asScala.toSeq
      verticesUnderV1.size shouldEqual 2
      verticesUnderV1(0).keySet.asScala shouldEqual Set(vertices(2))
      verticesUnderV1(0).values.asScala.head shouldEqual empty
      verticesUnderV1(1).keySet.asScala shouldEqual Set(vertices(3))

      val v3Edges = verticesUnderV1(1).values.asScala.head
      v3Edges.keySet.asScala shouldEqual Set(edges(2))

      val verticesUnderV3 = v3Edges.values.asScala.toSeq
      verticesUnderV3.size shouldEqual 1
      verticesUnderV3(0).keySet.asScala shouldEqual Set(vertices(4))
      verticesUnderV3(0).values.asScala.head shouldEqual empty
    }
  }
  ...
  "optional step".which {
    "doesn't take a default value" should {
      "return result of optional traversal if it has one" in new Fixture {
        ...
        results shouldEqual List("vadas", "josh")
      }
      ...
      "return identity if optional traversal doesn't find anything" in new Fixture {
        ...
        results shouldEqual List("marko")
      }
    }
  ...
  "steps to add things".can {
    "add an (unconnected) vertex for each path in the traversal".which {
      "has a label and properties" in new Fixture {
        ...
        graph
          ...
          .iterate()
        graph.V().hasLabel("newLabel").count().head() shouldEqual 2
        graph.V().has(NewProperty -> "someValue").count().head() shouldEqual 2
      }
    }
    ...
    "add elements".which {
      ...
      "show simple case" in new Fixture {
        ...
        g.V(michael).addE("loves").to(karlotta).iterate()
        g.addE("knows").from(michael).to(karlotta).iterate()
        michael.start().outE().count().head() shouldEqual 2
      }
      ...
      "add multiple edges in one iteration" in new Fixture {
        ...
        g.addE("knows")
          ...
          .to(michael)
          .iterate()
        michael.start().bothE().count().head() shouldEqual 4
      }
      ...
      "use multiple traversals" in new Fixture {
        ...
        graph.V(1).out(CoDeveloper).value(Name).toSet() shouldEqual Set("josh", "peter")
      }
      ...
      "reference the `to` vertex via StepLabel" in new Fixture {
        graph
          ...
          .to(v1Label)
          .iterate()
        graph.V(1).in(CoDeveloper).value(Name).toSet() shouldEqual Set("josh", "peter")
      }
      ...
      "reference the `to` vertex via traversal" in new Fixture {
        ...
        graph.V(1).outE(KnowsCreator).count().head() shouldEqual 1
      }
      ...
      "reference the `from` vertex via traversal" in new Fixture {
        ...
        // note: when using with addE, it only selects the first vertex!
        // https://groups.google.com/forum/#!topic/gremlin-users/3YgKMKB4iNs
        graph.V(1).inE(CreatorKnows).count().head() shouldEqual 1
      }
    }
  }
  ...
  "set properties".which {
    "have a given value" in new Fixture {
      ...
      graph.V().has(Name, "Joseph").count().head() shouldEqual 1
    }

    "have a given value (alternative step)" in new Fixture {
      graph.addV(Person).property(Name -> "Joseph2").iterate()
      graph.V().has(Name, "Joseph2").count().head() shouldEqual 1
    }
    ...
    "derive the value from a traversal" in new Fixture {
      ...
      get("marko") shouldEqual 1
      get("josh") shouldEqual 2
      get("peter") shouldEqual 1
    }
  }

  "local step".can {
    "limit a local subquery" in new Fixture {
      graph.V().outE().inV().count().head() shouldEqual 6
      graph.V().local(_.outE().limit(1)).inV().limit(3).count().head() shouldEqual 3
    }
  }
  ...
  "allows to be cloned" in new Fixture {
    ...
    gs.head() shouldEqual 1
    gsCloned.head() shouldEqual 1
  }
  ...
  "math step".can {
    ...
    "work with sideeffect key and placeholder" in new Fixture {
      graph
        ...
        .value(Age)
        .math("_ / x")
        .toSet() shouldEqual Set(2.9, 2.7, 3.2, 3.5)
    }
  }
